...
```json
{
  ...
  "occurredAt": "2026-07-21T12:12:45.123+09:00",
...
- `occurredAt`은 브라우저 위치와 무관하게 `Asia/Seoul`의 `+09:00` 오프셋을 명시한 ISO 8601 시각이다.
- 서버는 별도의 `serverTimestamp`를 같은 `+09:00` 형식으로 기록한다.
- 모든 분석 시각대는 `Asia/Seoul`을 사용한다.
...
| 열 |
| --- |
| `server_timestamp` (서버 수신 시각, `+09:00` ISO 8601 문자열) |
| `occurred_at` |
| `event_id` |
| `event_name` |
| `participant_id` |
